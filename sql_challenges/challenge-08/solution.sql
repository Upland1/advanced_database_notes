...
-- No, Too many matching rows (~20%)
-- Index would cause:
-- * index lookup and table access (expensive)
...
-- There's a full tabe scan because it covers almost all of the rows

-- d) Why does the range size affect whether Oracle uses the index?
-- Because of selectivity, if many rows match, the full scan wins
...
-- Yes, matches with the index structure:
-- *patient_id (equality)
-- *visit_Date (range)
...
-- No, the index is not used. Without patient_id, oracle cannot eddiciently navigate the index 
...
-- Because the index stores: patient_id = 5432, but query asks: TO_CHAR(patient_id). 
...
-- *order_status has 4 values as Cardinality
-- Concern:
-- *Each insert updates indexes so slows system
...
-- Why:
-- *Unique values so it turns very selective
-- *Fast lookup
