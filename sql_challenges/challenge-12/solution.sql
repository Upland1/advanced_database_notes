...
-- Which teams complete work faster over time?
--
...
-- Not all tasks have equal complexity or effort.
--
...
SELECT
    ...
    CASE
        ...
        (
            ...
            FROM (
                SELECT
                    COUNT(CASE WHEN t2.status = 'completed' THEN t2.id END)
                    / NULLIF(COUNT(DISTINCT u2.id), 0) AS team_velocity
                FROM teams tm2
                LEFT JOIN users u2 ON u2.team_id = tm2.id
                LEFT JOIN tasks t2 ON t2.assigned_to = u2.id
                GROUP BY tm2.id
            )
        )
        THEN 'Below Average'
...
ORDER BY velocity_per_member DESC;
...
-- ============================================================
-- 1. What is the business question?
-- How often are tasks completed before their deadline?
--
-- 2. What is the exact definition? (Include every filter, every join)
-- A task is on time if completed_at <= due_date + end of day.
-- Only completed tasks with non-null due dates are included.
--
-- 3. What are the edge cases? (NULLs, cancelled tasks, unassigned tasks, etc.)
-- Tasks without due dates are excluded.
-- Tasks completed minutes after midnight are considered late.
--
-- 4. What is the unit? (Count, percentage, hours, dollars?)
-- Percentage of completed tasks delivered on time.
--
-- 5. What would make this metric misleading?
-- Simple tasks and critical tasks are treated equally.
--
-- Write your answers as SQL comments above each query.
-- A query without a contract is just a number. A query WITH a contract
-- is a metric the business can trust.
...
--
-- PROBLEM:
-- Counting all assigned tasks does not measure productivity.
-- It ignores completion status, task complexity, and priority.
--
...
-- Task IDs are identifiers, not measurable performance values.
--
...
--
-- PROBLEM:
-- priority is VARCHAR2 and cannot be multiplied numerically.
-- Adding dates directly to strings/numbers creates meaningless values.
--
